...
class PhysicsSegment:
    ...
    def __post_init__(self) -> None:
        ...
        # derivative 与 equations 互斥
        mode_count = sum(1 for x in [self.derivative, self.equations] if x is not None)
        if mode_count > 1:
            raise ValueError(
                f"Segment '{self.segment_id}' cannot define both derivative and equations."
            )
